...
class AugmentOp:
    ...
    def _apply_augment(
        self, img, rand_val=None, magnitude=None, level_args=None
    ):
        if rand_val is None:
            rand_val = random.random()
        if self.prob < 1.0 and rand_val > self.prob:
            return img
    ...
    def __call__(self, img):
        if isinstance(img, list):
            if self.synchronized:
                ...
                for i in range(len(img)):
                    img[i] = self._apply_augment(img[i])
            return img
        else:
            return self._apply_augment(img)
    ...
    def __call__(self, img):
        ...
        if isinstance(img, list):
            if isinstance(img[0], np.ndarray):
                img = [Image.fromarray(img.astype("uint8")) for img in img]
                convert_back = True
